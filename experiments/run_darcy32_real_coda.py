...
print("=" * 100)
print("CoDA 真实数据集验证 - Darcy 32x32")
print("=" * 100)
...
try:
    ...
    print(f"✅ 成功导入 CoDA 模块")
except ImportError as e:
    print(f"❌ 导入 CoDA 模块失败: {e}")
    print("尝试简化导入...")
    # 备用方案：直接定义 CoDA 模块
    MHF_TFNO_CoDA = None
    MHF_TFNO_Baseline = None
...
def measure_inference(
    ...
    if hasattr(model, 'use_coda') and model.use_coda:
        for block in model.fno_blocks.convs:
...
def run_darcy32_experiment():
    ...
    try:
        ...
        print(f"\n❌ 加载数据失败: {e}")
        print("尝试使用合成数据...")
        from experiments.run_quick_test import create_synthetic_darcy_data
        X_train, Y_train = create_synthetic_darcy_data(500, 32, device)
        X_test, Y_test = create_synthetic_darcy_data(100, 32, device)
    ...
    for test_config in test_configs:
        ...
        try:
            ...
            if test_config['variant'] == 'baseline':
                # Baseline: 原始 TFNO
                if MHF_TFNO_Baseline is not None:
                    model = MHF_TFNO_Baseline(
                ...
                if MHF_TFNO_CoDA is not None:
                    model = MHF_TFNO_CoDA(
                        ...
                        mhf_rank=config['mhf_rank'],
                        use_coda=False,  # 不启用 CoDA
                    )
                else:
                    from neuralop.models import TFNO
                    model = TFNO(
...
def generate_report(results_file: Path, output_file: Path):
    ...
    if mhf_coda_result and baseline_result:
        if mhf_coda_result['best_test_loss'] < baseline_result['best_test_loss']:
            ...
            if mhf_coda_result['epochs_to_90'] and baseline_result:  # mhf_result['epochs_to_90']:
