...
| TFNO+CoDA (优化后) | 1.0X | 1.0X | 1.498X (33.4% 更快) | 99.3% (0.7% 精度损失) |

### CoDA 优化详情 (v1.1.0)

- **CrossHeadAttention 优化**: 采用轻量级 SE (Squeeze-and-Excitation) 风格的跨头注意力
- **torch.compile 支持**: 全面支持 PyTorch 2.0+ 编译加速
- **实测性能**: Darcy 32x32 数据集上
  - 推理延迟降低 33.4% (2.97ms → 1.98ms)
  - 精度损失仅 0.7% (可接受范围)
  - 完全兼容现有 TFNO+MHF 架构
